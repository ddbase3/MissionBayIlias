...
final class IliasEmbeddingQueueExtractorAgentResource extends AbstractAgentResource implements IAgentContentExtractor {
        ...
        public function extract(IAgentContext $context): array {
                ...
                foreach ($jobs as $job) {
                        ...
                        if ($jobId <= 0 || !$this->isHex32($cidHex) || $kind === '') {
                                $this->failFromJobRow($job, 'Extractor: missing job_id/content_id/source_kind', false);
                                continue;
                        }
        ...
        private function buildItemFromJob(array $job, array $providersByKind): ?AgentContentItem {
                ...
                $contentUuidHex = $cidHex; // canonical
                ...
                $subtreeRefIds = $containerObjId ? $this->treeResolver->getAllSubtreeRefIdsByObjId($containerObjId) : [];

                // NEW: direct UI link (store in metadata for every chunk)
                $directLink = '';
                try {
                        $directLink = $provider->getDirectLink($locator, $containerObjId, $sourceIntId);
                } catch (\Throwable) {
                        // keep empty; extractor must not fail just because link building failed
                        $directLink = '';
                }
                ...
                $meta['subtree_ref_ids'] = $subtreeRefIds;
